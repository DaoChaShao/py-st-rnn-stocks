...
norm_sessions: list[str] = ["norm", "y_norm"]
            ...
            if session_state["loaded_model"] is None:
                ...
                if button("Load the Trained Model", type="primary", width="stretch"):
                    with spinner("Loading Trained Model...", show_time=True, width="stretch"):
                        with Timer("Load the trained model") as session_state["loadTimer"]:
                            ...
                            # session_state["loaded_model"].compile(
                            #     optimizer="adam",
                            #     loss="mse",
                            #     metrics=[
                ...
                if session_state["y_pred"] is None:
                    ...
                    y_norm: list[str] = multiselect(
                        ...
                        default=session_state["y_norm"], disabled=True,
                    ...
                    comparison: DataFrame = DataFrame({
                        f"{', '.join(session_state["y_norm"])} Ture": session_state["y_test"].flatten(),
                        f"{', '.join(session_state["y_norm"])} Pred": session_state["y_pred"].flatten(),
                    }).iloc[start: end + 1]
